...
export default function WatchlistClient({ watchlistItems, initialStocks, quotes, profiles, metrics, news, activeAlerts }: WatchlistClientProps) {
  const { data: watchlist } = useWatchList(watchlistItems)
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        ...
        <SearchCommand quickAdd={true} renderAs="button" initialStocks={initialStocks} watchlistSymbols={watchlist} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="watchlist-alerts flex">
          <div className="flex items-center gap-2 mb-4">
            ...
            {alerts.length > 0 && <span className="ml-auto text-xs font-mono text-gray-500 bg-gray-700 px-2 py-0.5 rounded-full">{alerts.length}</span>}
          </div>
